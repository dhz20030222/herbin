...
def calculate_intersection():
    ...
    for _, row in grib_df.iterrows():
        ...
        if grid_poly.intersects(basin_geom):
            ...
            # 计算该小区域的降水量
            precip = row['tp'] * 1000  # 转换为毫米
            area_precip = precip * intersection_area  # 该小区域的总降水量

            total_precip += area_precip
            ...
            results.append({
                ...
                'area_precip': area_precip,  # 小区域降水量
                'area_avg_precip': area_precip / intersection_area if intersection_area > 0 else 0  # 小区域平均降水量
            })

    result_df = pd.DataFrame(results)

    # 计算整个流域的平均降水量
    basin_avg_precip = total_precip / total_area if total_area > 0 else 0
    ...
    print(f"流域总面积: {total_area:.2f}")
    print(f"流域平均降水量: {basin_avg_precip:.2f}mm")
    print(f"最大小区域降水量: {result_df['area_precip'].max():.2f}mm")
    print(f"最小小区域降水量: {result_df['area_precip'].min():.2f}mm")
